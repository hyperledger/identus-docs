...
- **API Key Length:** To maintain robust security, the length of the APIKey value must exceed 16 bytes (128 bits). This
  ...
  **Unique API Keys:** Each API Key is unique to a specific entity. It cannot be shared or reused by other entities. If
  ...
  considered unusable.
- **Revocation:** In cases where an APIKey is revoked for a tenant, it becomes invalid and cannot be used for
  authentication.
...
Automatically register the Entity, Wallet, and APIKey for the tenant during the first interaction with the Cloud Agent
over the REST API.
...
resource W. In this model, user A is a registered user in Keycloak, and wallet W is a resource registered in Keycloak.
...
wallet-{wallet_id} in Keycloak. Similarly, for each user and resource, a corresponding policy is created with the name
user {userId} on wallet {wallet_resource_name} permission.

...
### DID Url
A DID itself is a type of a URL, while `did` is a registered schema type, like `http` and `https`. In identus system, we refer DID urls to a DID that includes path and query paramteres and can resolve a resource via one of the service endpoints in the DID document. for example: 
```
